...
public class MyDatabase {
    ...
    public ArrayList<Data> getarray(){            //获取listview中要显示的数据
        ...
        Cursor cursor = mydatabase.rawQuery("select ids,title,content,times from mynotes",null);
        cursor.moveToFirst();
        while(!cursor.isAfterLast()){
            ...
            Data data = new Data(title,id,content,times);
            arr.add(data);
            cursor.moveToNext();
        }
        mydatabase.close();
    ...
    public ArrayList<Data> search_array(String a){            //获取listview中要显示的数据
        ArrayList<Data> arr = new ArrayList<Data>();
        ArrayList<Data> arr1 = new ArrayList<Data>();
        mydatabase = myOpenHelper.getWritableDatabase();
        Cursor cursor = mydatabase.rawQuery("select ids,title,content,times from mynotes",null);
        ...
        for(int i=0;i<arr.size();i++){
            if((arr.get(i).getContent().indexOf(a)!=-1)||(arr.get(i).getTitle().indexOf(a)!=-1))
            {
                arr1.add(arr.get(i));
            }
        }
    ...
    public void toUpdate(Data data){           //修改表中数据
        mydatabase = myOpenHelper.getWritableDatabase();
        mydatabase.execSQL(
                "update mynotes set title='"+ data.getTitle()+
                        "',times='"+data.getTimes()+
